...
- SQL Conversion 보조 매핑 지원: `NEXT_SQL_COMPLEX_MAP`의 FR_TABLE 정확 일치 룰 전체와 다른 FR_TABLE RAG top-k 룰을 `tobe_sql_prompt.json`에 함께 전달
...
현재 cycle당 agent별 실행 대상 수는 `.env`의 `MIGRATION_JOB_BATCH_SIZE`, `SQL_CONVERSION_JOB_BATCH_SIZE`, `SQL_TUNING_JOB_BATCH_SIZE`, `SQL_FORMATTING_JOB_BATCH_SIZE`로 관리합니다. SQL row의 재처리 상한은 `.env`의 `JOB_MAX_BATCH_COUNT`로 관리합니다.
...
## SQL Conversion supplemental mapping

SQL conversion 단계에서는 simple/complex flow를 나누지 않고 `server/config/prompts/tobe_sql_prompt.json` 하나를 사용합니다.

- 기본 매핑룰은 항상 `NEXT_MIG_INFO` / `NEXT_MIG_INFO_DTL`에서 가져옵니다.
- `NEXT_SQL_INFO.TARGET_TABLE`은 JSON list 또는 구분 문자열로 저장된 AS-IS table 목록이며, 이 값이 각 매핑 테이블의 `FR_TABLE` 매칭 기준입니다.
- `NEXT_SQL_COMPLEX_MAP`은 DB Migration 공통 매핑이 아니라 SQL Conversion 전용 보조 매핑룰 저장소입니다.
- `NEXT_SQL_COMPLEX_MAP`에는 `MAP_KIND`, `GENERAL`, `SEARCH` 구분을 사용하지 않습니다.
- `TARGET_TABLE`에 포함된 table과 `NEXT_SQL_COMPLEX_MAP.FR_TABLE`이 일치하는 룰은 전부 프롬프트에 넣습니다.
- `TARGET_TABLE`과 `FR_TABLE`이 다른 룰은 전체 후보 중 source SQL 기준 RAG top-k만 추가합니다. 기본값은 `COMPLEX_MAP_OTHER_TOP_K=2`입니다.
- `mapping_schema_text`는 `[MIGRATION_MAPPING_RULES]`, `[SQL_CONVERSION_SUPPLEMENTAL_RULES_EXACT_ALL_AND_OTHER_TOP_K]`를 함께 전달합니다.
- LLM prompt에는 `MAP_ID`, 검색 점수 같은 내부 메타데이터를 전달하지 않습니다.
- `COMPLEX_MAP_EXACT_MATCH_INCLUDE_ALL`과 `COMPLEX_MAP_OTHER_TOP_K`로 보조 매핑 포함 범위를 조정합니다.
- `NEXT_SQL_COMPLEX_MAP`의 `USE_YN`, `FR_TABLE`은 대문자/공백 없는 값으로 저장해야 합니다. 조회 쿼리는 인덱스 사용을 위해 컬럼에 `UPPER(TRIM())`을 적용하지 않습니다.
...
Supervisor, SQL Conversion, DB Migration LLM 호출에서 `401 team not allowed to access model`, `model not allow`, `model not found` 같은 치명적인 model access 오류가 발생하면 `LLM_FALLBACK_MODELS` 순서대로 다음 모델을 시도합니다.
...
COMPLEX_MAP_EXACT_MATCH_INCLUDE_ALL=true
COMPLEX_MAP_OTHER_TOP_K=2
