...
- SQL Conversion 보조 매핑 지원: `NEXT_SQL_COMPLEX_MAP`의 FR_TABLE별 RAG top 3 룰을 `tobe_sql_prompt.json`에 함께 전달
...
- `TARGET_TABLE`에 포함된 table별로 `NEXT_SQL_COMPLEX_MAP.USE_YN='Y' AND FR_TABLE=target_table` 후보를 가져온 뒤, 해당 후보 안에서 source SQL 기준 RAG top 3을 선택합니다.
- `mapping_schema_text`는 `[MIGRATION_MAPPING_RULES]`, `[SQL_CONVERSION_SUPPLEMENTAL_RULES_TOP_3_BY_FR_TABLE]`를 함께 전달합니다.
- LLM prompt에는 `MAP_ID`, 검색 점수 같은 내부 메타데이터를 전달하지 않습니다.
- `COMPLEX_MAP_SEARCH_TOP_K`로 table별 보조 매핑 검색 건수를 조정합니다.
