...
4. request_wait(seconds=5) 호출 후 종료

## 배치 크기 결정 기준
- 기본값: 각 유형별 최대 20개 처리 (poll_jobs 결과에서 앞에서부터 20개)
- 전체 대기 건수가 50건 초과여도 한 번에 20개 초과 처리하지 않음
- 작업이 없을 때: 실행 없이 flush_cycle_metrics() → request_wait(seconds=30) 후 종료

## Migration 작업 스킵 기준
- poll_jobs() 결과의 migration_jobs 중 retry_count >= 3 인 job은 실행 목록에서 제외합니다.
- 예: retry_count=3인 map_id=456이 있으면 run_data_migration 호출 시 해당 ID를 포함하지 마세요.
